...
const getSubfolders = (rootDir) => {
  if (!fs.existsSync(rootDir)) {
    console.warn(`The directory ${rootDir} does not exist. It will be created.`);
  ...
  plop.setGenerator("react-component", {
    ...
    prompts: [
      ...
      },
      {
        type: "list",
        name: "directory",
        message: "Choose an existing sub-folder, create a new one, or use src as root:",
        choices: () => {
    ...
    actions: (data) => {
      ...
      if (data.directory === "(Create new folder)") {
        ...
        if (!fs.existsSync(newFolderPath)) {
          ...
          packageExportContent = [{
            ...
            template: '    "./{{directory}}": "./src/{{directory}}/index.ts",',
            unique: true,
          }];
        }
      ...
      return [
        ...
        },
        {
          type: "append",
          path: "src/{{directory}}/index.ts",
          template: 'export { default as {{pascalCase name}} } from "./{{kebabCase name}}.tsx";',
        },
        ...packageExportContent,
      ];
    },
